<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="left" :label="$t('user.reg_number')">
        <template slot-scope="scope">
          <span>{{ scope.row.no_pendaftaran }}</span>
        </template>
      </el-table-column>

      <el-table-column align="left" :label="$t('user.name')">
        <template slot-scope="scope">
          <span>{{ scope.row.name }} {{ scope.row.nama_belakang }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="Actions" width="350">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="scope.row.roles.includes('students')"
            v-permission="['manage user']"
            type="danger"
            size="small"
            icon="el-icon-delete"
...
import SantriResource from '@/api/santri';
import Resource from '@/api/resource';
import waves from '@/directive/waves'; // Waves directive
import permission from '@/directive/permission'; // Permission directive
import checkPermission from '@/utils/permission'; // Permission checking

const userResource = new SantriResource();
...
export default {
  ...
  data() {
    ...
    return {
      ...
      roles: ['SMP', 'SMA', 'MTS', 'MA'],
      nonAdminRoles: ['SMP', 'SMA', 'MTS', 'MA'],
      newUser: {},
